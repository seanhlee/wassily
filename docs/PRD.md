...
## Ramp Generation (v6 Semantic Solver)

### Architecture

The opinionated ramp engine uses a **seed-constrained OKLab path solver** followed by a **semantic tonal role envelope**.

The core insight is still perceptual geometry: color movement should be solved in OKLab, not by independently easing L, C, and H. The v6 solver extends that idea with a stronger design model:

- The exact seed color is a hard constraint.
- The path is optimized before stop sampling using perceptual density, curvature, hue stability, seed-exit behavior, and corpus-conditioned soft priors.
- The 11-stop ladder is semantic. `50` means tinted paper, `950` means colored ink, and the intermediate labels should arrive there with perceptual cadence.
...
The implementation lives in `src/engine/v6ResearchSolver.ts`; the visual research boards are generated by `npm run research:lab` and `npm run research:gauntlet`.
...
The user's seed color must appear exactly in the generated ramp. The solver chooses the nearest useful semantic stop from the solved path, then preserves the exact seed after role shaping.

This lets a bright lime naturally anchor around `200`, a mid phthalo green around `500`, and a deep ultramarine around its body stop without inventing a separate ramp type for each case.

#### 2. Semantic Tint and Ink Roles

The endpoints are not treated as arbitrary math extremes:

- **50**: tinted paper. It is light, but not empty white; current chromatic ramps target about `L 0.958`.
- **100/200**: the highlight bridge. For extreme high-lightness chroma, these stops must avoid a cliff from colored seed to pale tint.
- **900/950**: colored ink. The tail should retain hue identity instead of collapsing into black.
- **Neutral 50/950**: quiet paper and quiet ink. Neutrals use lower chroma envelopes and should not become colorful just because the gamut allows it.

The endpoint model is family-aware:

- Bright lime gets cusp-aware handling so `50/100/200` stay beautiful despite extreme high-lightness chroma.
...
**Ramp generation (v6):** Solve a seed-constrained OKLab path with exact seed placement, perceptual density scoring, curvature/hue stability pressure, and corpus-conditioned soft priors. For the 11-stop ladder, apply semantic tonal roles: chromatic `50` as tinted paper, `950` as colored ink, neutral-specific quiet endpoints, lime cusp handling, cyan/phthalo tint occupancy, and blue-violet ink cadence. Clamp to gamut. Generate parallel dark mode variant.
